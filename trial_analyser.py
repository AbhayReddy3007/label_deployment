        ...
        for attempt in range(GEMINI_MAX_RETRIES):
            try:
                ...
                if _is_transient(exc) and attempt < GEMINI_MAX_RETRIES - 1:
                    ...
                    logger.warning("[TRIAL_ANALYSER] %s — retrying in %ss (%d/%d)", exc, delay, attempt + 1, GEMINI_MAX_RETRIES)
                    time.sleep(delay)
                elif i == 0 and len(configs) > 1:
                    logger.info("[TRIAL_ANALYSER] Error with Search grounding (%s) — trying without grounding", exc)
                    break
                else:
                    raise
        if i == 0 and len(configs) > 1:
            logger.info("[TRIAL_ANALYSER] Empty/failed response with Search grounding — retrying without grounding")
